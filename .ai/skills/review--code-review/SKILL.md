...
### 🔍 動作確認の提案 (Verification Guidance)

| チェック項目 | 確認内容 |
|-------------|---------|
| 手動テスト手順 | 変更が正しく動作することを確認するための具体的な操作手順 |
| 自動テストの網羅性 | テストが追加・更新されているか、不足しているテストはないか |
| エッジケース | 境界値や異常系の入力で正しく動作するか |
| リグレッションリスク | 既存機能への影響がないか、実行すべき回帰テストは何か |
| 環境依存の確認事項 | DB変更・設定変更・環境変数など環境固有の確認が必要か |
| PR記載内容との照合 | PRの「動作確認」セクションの記載が十分か、漏れがないか |
...
7. **動作確認方法を考える** - コードの正しさだけでなく、どう検証するかも提案
