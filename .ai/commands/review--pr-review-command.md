---
description: PRの内容と差分をチェックし変更を要約・レビュー箇所を提案
model: inherit
subtask: true
---
...
7. PRの説明文に「動作確認」セクションがあればその内容を確認し、記載内容が十分か評価する
8. 差分の内容を踏まえ、レビュワーが変更の正しさを検証するための具体的な動作確認方法を提案する
   - 手動で確認すべき操作手順
   - 確認すべきエッジケース・異常系
   - 実行すべき回帰テスト
   - 不足しているテストの追加案
...
- レビュー結果には「動作確認の提案」セクションを含め、レビュワーが実際に動作検証できる具体的な手順を示す
